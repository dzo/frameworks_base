...
public abstract class DataConnection extends StateMachine {
   ...
   //***** Constructor
     ...
     * @param isPartialRetryResponse is partial retry response flag
     * @return SetupResult.
     */
    private DataCallState.SetupResult onSetupConnectionCompleted(AsyncResult ar,
            boolean isPartialRetryResponse) {
        ...
        if (ar.exception != null) {
            ...
            result = DataCallState.SetupResult.ERR_Stale;
                    // If it is partial retry response - anything except 0 is a failure
        } else if ((isPartialRetryResponse && response.status != 0) ||
                 // If it is NOT partial retry response (i.e. initial setup_data_call)
                 // then partial retry failure is still a success (if somehow it happened
                 // that modem and apps are out of sync and there is partially succesful
                 // data call in modem, but not in apps)
                (!isPartialRetryResponse &&
                response.status != FailCause.PARTIAL_RETRY_FAILURE.getErrorCode() &&
                response.status != FailCause.NONE.getErrorCode())) {
            result = DataCallState.SetupResult.ERR_RilError;
            result.mFailCause = FailCause.fromInt(response.status);
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case EVENT_SETUP_DATA_CONNECTION_DONE:
                    ...
                    DataCallState.SetupResult result = onSetupConnectionCompleted(ar, false);
    ...
    private class DcActiveState extends State {
        ...
        private void handlePartialRetrySetupRequestCompleted(Message msg) {
            ...
            DataCallState.SetupResult result = onSetupConnectionCompleted(ar, true);
            if (DBG) {
